...
const notifications = [
    ...
    { id: 3, type: 'interview', text: 'TechCorp interview scheduled for Friday.', time: '2 days ago' },
    { id: 4, type: 'announcement', text: 'College will be closed for a national holiday.', time: '3 days ago' },
];

const dayWiseTimetable = {
    "Monday": [{ time: "9-10 AM", subject: "Data Structures", teacher: "Dr. Evelyn Reed" }, { time: "10-11 AM", subject: "Algorithms", teacher: "Prof. Ben Carter" }],
    "Tuesday": [{ time: "11-12 PM", subject: "Database Systems", teacher: "Dr. Chloe Green" }, { time: "2-3 PM", subject: "Operating Systems", teacher: "Prof. David Shaw" }],
    "Wednesday": [{ time: "9-10 AM", subject: "Data Structures", teacher: "Dr. Evelyn Reed" }, { time: "10-11 AM", subject: "Algorithms", teacher: "Prof. Ben Carter" }],
    "Thursday": [{ time: "11-12 PM", subject: "Database Systems", teacher: "Dr. Chloe Green" }, { time: "2-3 PM", subject: "Operating Systems", teacher: "Prof. David Shaw" }],
    "Friday": [{ time: "1-2 PM", subject: "Project Lab", teacher: "Dr. Evelyn Reed" }],
    "Saturday": [],
    "Sunday": [],
};

const assignments = [
    { id: 1, subject: 'Data Structures', title: 'Binary Tree Implementation', dueDate: '2024-08-15', status: 'Submitted', feedback: 'Good work. A bit more optimization possible in the search function.' },
    { id: 2, subject: 'Database Systems', title: 'Normalization Assignment', dueDate: '2024-08-20', status: 'Pending', feedback: null },
    { id: 3, subject: 'Operating Systems', title: 'Process Scheduling Simulation', dueDate: '2024-08-10', status: 'Graded', feedback: 'Excellent implementation of the FCFS algorithm.' },
];

const grades = [
    { subject: 'Data Structures', internal: 25, external: 65, total: 90, grade: 'A+', credits: 4 },
    { subject: 'Algorithms', internal: 22, external: 60, total: 82, grade: 'A', credits: 4 },
    { subject: 'Database Systems', internal: 28, external: 70, total: 98, grade: 'O', credits: 3 },
    { subject: 'Operating Systems', internal: 20, external: 55, total: 75, grade: 'B+', credits: 3 },
];

const attendanceData = [
    { name: 'Jan', Attendance: 90 }, { name: 'Feb', Attendance: 85 }, { name: 'Mar', Attendance: 74 },
    { name: 'Apr', Attendance: 92 }, { name: 'May', Attendance: 88 }, { name: 'Jun', Attendance: 72 },
];
...
const financeDetails = {
    ...
    due: 45000,
    installments: [
        { name: 'Installment 1', amount: 50000, status: 'Paid', dueDate: '2024-07-01' },
        { name: 'Installment 2', amount: 50000, status: 'Paid', dueDate: '2024-01-15' },
        { name: 'Installment 3', amount: 25000, status: 'Due', dueDate: '2024-08-30' },
    ],
    history: [
        { id: 1, date: '2024-07-01', amount: 50000, type: 'Installment 1', receipt: '#' },
        { id: 2, date: '2024-01-15', amount: 25000, type: 'Partial - Inst. 2', receipt: '#' },
    ]
};

const jobListings = [
    { id: 1, role: 'Frontend Developer', company: 'Innovate Inc.', location: 'Remote' },
    { id: 2, role: 'Backend Engineer', company: 'Data Solutions', location: 'New York' },
    { id: 3, role: 'UI/UX Designer', company: 'Creative Minds', location: 'San Francisco' },
];

const calendarEvents = [
    { id: 1, title: 'Mid-Term Exams', date: '2024-09-05', type: 'academic' },
    { id: 2, title: 'Tech Fest "Innovate"', date: '2024-09-20', type: 'cultural' },
    { id: 3, title: 'Placement Drive: TechCorp', date: '2024-08-25', type: 'placement' },
];
...
const DashboardOverview = () => (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <StatCard icon={<Calendar size={24} className="text-yellow-600" />} title="Upcoming Events" value={studentData.stats.upcomingEvents} color="bg-yellow-100" />
            <StatCard icon={<Briefcase size={24} className="text-purple-600" />} title="Placement Updates" value={studentData.stats.placementUpdates} color="bg-purple-100" />
        </div>
            ...
            <div className="space-y-4 max-h-60 overflow-y-auto pr-2">
                {notifications.map(n => (
                    <div key={n.id} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                        <div className="p-2 bg-blue-100 rounded-full"><Bell size={16} className="text-blue-600" /></div>
                        <div>
...
const Academics = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'Marks & Grades': return <MarksAndGrades />;
        ...
        <Card>
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
                    <button onClick={() => setView('Day')} className={`px-3 py-1 text-sm rounded-md ${view === 'Day' ? 'bg-white shadow' : ''}`}>Day</button>
                    <button onClick={() => setView('Week')} className={`px-3 py-1 text-sm rounded-md ${view === 'Week' ? 'bg-white shadow' : ''}`}>Week</button>
                </div>
            </div>
            {view === 'Day' && (
                <div>
                    <select value={currentDay} onChange={(e) => setCurrentDay(e.target.value)} className="mb-4 p-2 border rounded-md w-full md:w-auto">
                        {days.map(day => <option key={day} value={day}>{day}</option>)}
                    </select>
                    <div className="space-y-3">
                        {dayWiseTimetable[currentDay]?.length > 0 ? dayWiseTimetable[currentDay].map((slot, i) => (
                            <div key={i} className="p-4 bg-blue-50 rounded-lg flex justify-between items-center">
                                <div>
                                    <p className="font-bold text-blue-800">{slot.subject}</p>
                                    <p className="text-sm text-blue-600">{slot.teacher}</p>
                                </div>
                                <p className="text-sm font-medium text-gray-700">{slot.time}</p>
                            </div>
                    ...
                    <table className="w-full text-left min-w-[600px]">
                        <thead>
                            <tr className="bg-gray-50">
                                ...
                                <th className="p-3 text-sm font-semibold text-gray-600">Schedule</th>
                            </tr>
                        </thead>
                        <tbody>
                            {days.map(day => (
                                <tr key={day} className="border-b">
                                    ...
                                    <td className="p-3">
                                        {dayWiseTimetable[day]?.length > 0 ? dayWiseTimetable[day].map(s => `${s.time}: ${s.subject}`).join(', ') : <span className="text-gray-400">No classes</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div key={a.id} className="p-4 border rounded-lg">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">{a.subject} | Due: {a.dueDate}</p>
                        </div>
                        <StatusChip status={a.status} />
                    </div>
                    {a.feedback && <p className="text-sm mt-2 p-2 bg-gray-100 rounded-md"><strong>Feedback:</strong> {a.feedback}</p>}
                    <div className="flex space-x-4 mt-3">
                        <button className="flex items-center space-x-2 text-sm text-blue-600 hover:underline"><Upload size={16} /> <span>Upload</span></button>
                        <button className="flex items-center space-x-2 text-sm text-blue-600 hover:underline"><Download size={16} /> <span>Download</span></button>
                    </div>
                </div>
            ))}
        </div>
...
const MarksAndGrades = () => {
    ...
    const gradeToPoint = { 'O': 10, 'A+': 9, 'A': 8, 'B+': 7, 'B': 6, 'C': 5, 'P': 4, 'F': 0 };

    const totalCredits = grades.reduce((acc, g) => acc + g.credits, 0);
    const totalPoints = grades.reduce((acc, g) => acc + (gradeToPoint[g.grade] * g.credits), 0);
    ...
    const GpaCalculator = () => {
        ...
        const addSubject = () => setSubjects([...subjects, { credits: '', grade: '' }]);
            ...
            <div className="space-y-4">
                {subjects.map((s, i) => (
                    <div key={i} className="flex items-center space-x-4">
                        <input type="number" placeholder="Credits" value={s.credits} onChange={e => handleSubjectChange(i, 'credits', e.target.value)} className="p-2 border rounded-md w-1/3" />
                        <select value={s.grade} onChange={e => handleSubjectChange(i, 'grade', e.target.value)} className="p-2 border rounded-md w-2/3">
                            <option value="">Select Grade</option>
                            {Object.keys(gradeToPoint).map(g => <option key={g} value={g}>{g}</option>)}
                        </select>
                    </div>
                ))}
                <button onClick={addSubject} className="text-sm text-blue-600 hover:underline">+ Add another subject</button>
                <div className="flex items-center justify-between pt-4">
                    <button onClick={calculate} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Calculate GPA</button>
                    {calculatedGpa !== null && <p className="text-xl font-bold">Estimated GPA: <span className="text-blue-600">{calculatedGpa}</span></p>}
                </div>
            </div>
        )
    }
                    ...
                    <table className="w-full text-left min-w-[500px]">
                        <thead>
                            <tr className="bg-gray-50">
                                ...
                                <th className="p-3 text-sm font-semibold text-gray-600">Total Marks</th>
                            </tr>
                        </thead>
                        <tbody>
                            {grades.map((g, i) => (
...
const AttendanceTracker = () => {
    ...
    return (
        <Card>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Attendance Tracker</h2>
            {overallAttendance < 75 && (
                <div className="p-4 bg-red-100 text-red-800 rounded-lg mb-4 flex items-center space-x-3">
                    <AlertTriangle size={20} />
                    <p><strong>Overall Alert:</strong> Your attendance is below 75%. Please ensure you meet the minimum requirement.</p>
                </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    <h3 className="font-semibold text-gray-700 mb-4">Subject-wise Attendance</h3>
                    <div className="space-y-3">
                        {subjectAttendance.map(sub => (
                             <div key={sub.subject} className="p-3 bg-gray-50 rounded-lg">
                                <div className="flex justify-between items-center">
                                    ...
                                    <p className={`font-bold ${sub.percentage < 75 ? 'text-red-600' : 'text-green-600'}`}>{sub.percentage}%</p>
                                </div>
                                {sub.percentage < 75 && (
                                    <div className="flex items-start space-x-2 mt-2 text-xs text-red-700 bg-red-50 p-2 rounded-md">
                                        <AlertTriangle size={14} className="flex-shrink-0 mt-0.5"/>
                                        <span>Warning: Low attendance. Please consult your subject teacher or attend remedial classes.</span>
                                    </div>
                                )}
                            </div>
                        ))}
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="text-center">
                    ...
                    <p className="text-3xl font-bold text-gray-800">₹{financeDetails.total.toLocaleString()}</p>
                </Card>
                <Card className="text-center">
                    <p className="text-sm text-gray-500">Total Paid</p>
                    <p className="text-3xl font-bold text-green-600">₹{financeDetails.paid.toLocaleString()}</p>
                </Card>
                <Card className="text-center">
                    <p className="text-sm text-gray-500">Amount Due</p>
                    <p className="text-3xl font-bold text-red-600">₹{financeDetails.due.toLocaleString()}</p>
                </Card>
            </div>
            <Card>
                <h2 className="text-xl font-bold text-gray-800 mb-4">Installment Plan</h2>
                <div className="space-y-2 mb-4">
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                        <div className="bg-green-600 h-2.5 rounded-full" style={{ width: `${paidPercentage}%` }}></div>
                    </div>
                    <p className="text-sm text-gray-600 text-right">{paidPercentage.toFixed(0)}% Paid</p>
                </div>
                <div className="space-y-3">
                    {financeDetails.installments.map((item, index) => (
                        <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            <div>
                                <p className="font-medium text-gray-800">{item.name} <span className="text-xs text-gray-500">(Due: {item.dueDate})</span></p>
                 ...
                 <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">Enable in-app and email notifications for upcoming fee deadlines.</p>
                        </div>
                    </div>
                    <button onClick={() => setReminders(!reminders)}>
                        {reminders ? <ToggleRight size={36} className="text-blue-600" /> : <ToggleLeft size={36} className="text-gray-400" />}
                    </button>
                 </div>
            </Card>
        </div>
                ...
                <div className="md:col-span-2 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="name" type="text" value={profileData.name} onChange={handleInputChange} readOnly={!isEditing} className={`w-full p-2 border rounded-md mt-1 transition-colors ${!isEditing ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`} />
                        </div>
                        <div>
                            <label className="text-sm text-gray-500">Department</label>
                            <input type="text" value={studentData.department} className="w-full p-2 border rounded-md mt-1 bg-gray-100 cursor-not-allowed" readOnly />
                        </div>
                        <div>
                            <label className="text-sm text-gray-500">Email</label>
                            <input name="email" type="email" value={profileData.email} onChange={handleInputChange} readOnly={!isEditing} className={`w-full p-2 border rounded-md mt-1 transition-colors ${!isEditing ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`} />
                        </div>
                        <div>
                            <label className="text-sm text-gray-500">Phone</label>
                            <input name="phone" type="tel" value={profileData.phone} onChange={handleInputChange} readOnly={!isEditing} className={`w-full p-2 border rounded-md mt-1 transition-colors ${!isEditing ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`} />
                        </div>
                    </div>
                    {isEditing && (
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-center space-x-2 bg-gray-100 p-1 rounded-xl">
                <button onClick={() => setActiveTool('CV')} className={`px-4 py-2 text-sm font-medium rounded-lg ${activeTool === 'CV' ? 'bg-white shadow' : 'text-gray-600'}`}>CV Analyzer</button>
                <button onClick={() => setActiveTool('Study')} className={`px-4 py-2 text-sm font-medium rounded-lg ${activeTool === 'Study' ? 'bg-white shadow' : 'text-gray-600'}`}>Study Bot</button>
                <button onClick={() => setActiveTool('Prep')} className={`px-4 py-2 text-sm font-medium rounded-lg ${activeTool === 'Prep' ? 'bg-white shadow' : 'text-gray-600'}`}>Placement Prep</button>
            </div>
...
const CVAnalyzer = () => {
    ...
    return (
        <Card>
            <h2 className="text-xl font-bold text-gray-800 mb-4 text-center">CV Analyzer</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-xl">
                    <Upload size={48} className="text-gray-400 mb-4" />
                    <p className="text-gray-600 mb-4">Upload your resume to get instant feedback.</p>
                    <button className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Upload CV</button>
                </div>
                <div className="flex flex-col items-center">
                    <div className="relative w-40 h-40">
                        <svg className="w-full h-full" viewBox="0 0 36 36">
                            ...
                            <path className="text-purple-600" strokeWidth="3" strokeDasharray={`${score}, 100`} strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                        </svg>
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-3xl font-bold text-purple-700">{score}%</div>
                    </div>
                    <p className="mt-4 text-lg font-medium text-gray-700">Your CV Score</p>
                    <div className="mt-4 w-full space-y-2 text-sm">
                        <p className="flex justify-between"><span>Keywords:</span> <span className="font-semibold text-green-600">Excellent</span></p>
                        <p className="flex justify-between"><span>Formatting:</span> <span className="font-semibold text-yellow-600">Good</span></p>
                        <p className="flex justify-between"><span>Impact:</span> <span className="font-semibold text-green-600">Excellent</span></p>
                    </div>
                </div>
            </div>
        </Card>
...
const StudyBot = () => (
    <Card className="flex flex-col" style={{height: '65vh'}}>
        ...
        <div className="flex-grow bg-gray-50 rounded-lg p-4 space-y-4 overflow-y-auto">
            ...
            <div className="flex justify-end"><div className="bg-blue-500 text-white p-3 rounded-lg max-w-xs shadow-sm">Explain the concept of normalization in databases.</div></div>
        </div>
        <div className="mt-4 flex space-x-2">
            <input type="text" placeholder="Ask a question..." className="flex-grow p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <button className="p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex-shrink-0"><Send size={20} /></button>
        </div>
    </Card>
);
...
const PlacementPrepBot = () => (
    <Card>
        <h2 className="text-xl font-bold text-gray-800 mb-4 text-center">Placement Prep Bot</h2>
        <div className="p-6 border rounded-xl bg-gray-50">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                ...
                <button className="p-3 border rounded-lg hover:bg-gray-100">15 seconds</button>
            </div>
        </div>
        <div className="flex justify-center space-x-4 mt-4">
             <span className="flex items-center space-x-1 text-sm bg-green-100 text-green-800 px-3 py-1 rounded-full"><ThumbsUp size={14} /> Correct!</span>
             <span className="flex items-center space-x-1 text-sm bg-red-100 text-red-800 px-3 py-1 rounded-full"><ThumbsDown size={14} /> Try Again</span>
        </div>
    </Card>
);
...
const PlacementPortal = () => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            <Card>
                ...
                <div className="flex flex-wrap gap-4 mb-4">
                    <input type="text" placeholder="Role, Company" className="p-2 border rounded-md" />
                ...
                <div className="space-y-4">
                    {jobListings.map(job => (
                        <div key={job.id} className="p-4 border rounded-lg flex justify-between items-center">
                            ...
                            <button className="px-4 py-2 text-sm bg-blue-100 text-blue-800 rounded-lg hover:bg-blue-200">Apply</button>
                        </div>
                    ))}
                </div>
            </Card>
        ...
        <div className="space-y-6">
            <Card>
                <h2 className="text-xl font-bold text-gray-800 mb-4">Application Tracker</h2>
                <div className="space-y-3 text-sm">
                    <p className="flex justify-between"><span>Innovate Inc.</span><StatusChip status="Submitted" /></p>
                    <p className="flex justify-between"><span>Data Solutions</span><StatusChip status="Pending" /></p>
                </div>
            </Card>
             <Card>
                <h2 className="text-xl font-bold text-gray-800 mb-4">Quiz Leaderboard</h2>
                <div className="space-y-3 text-sm">
                    <p className="flex justify-between items-center"><span>1. Sarah Jenkins</span> <span className="font-bold">1500 pts</span></p>
                    <p className="flex justify-between items-center bg-blue-50 p-2 rounded-md"><span>2. Alex Thompson</span> <span className="font-bold">1450 pts</span></p>
                    <p className="flex justify-between items-center"><span>3. Mike Ross</span> <span className="font-bold">1420 pts</span></p>
                </div>
            </Card>
        </div>
    </div>
        ...
        <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {calendarEvents.map(event => (
                    <div key={event.id} className="p-4 border-l-4 border-purple-500 bg-purple-50 rounded-r-lg">
            ...
            <div className="md:col-span-1 border-r pr-4">
                ...
                <div className="space-y-2">
                    <div className="p-2 bg-blue-100 rounded-lg cursor-pointer">
                        ...
                        <p className="text-xs text-blue-600 truncate">Okay, I'll check your submission...</p>
                    </div>
                    <div className="p-2 hover:bg-gray-100 rounded-lg cursor-pointer">
                        <p className="font-medium text-gray-700">Placement Cell</p>
                        <p className="text-xs text-gray-500 truncate">Reminder: Resume submission deadline...</p>
                    </div>
                </div>
            </div>
            <div className="md:col-span-2">
                ...
                <div className="space-y-3">
                    <div className="p-3 bg-yellow-50 border-l-4 border-yellow-400">
                        ...
                        <p className="text-sm text-yellow-700">Fee portal will be down for maintenance on Sunday.</p>
                    </div>
                     <div className="p-3 bg-green-50 border-l-4 border-green-400">
                        <p className="font-medium text-green-800">CSE Department</p>
                        <p className="text-sm text-green-700">Guest lecture on "AI in Modern Computing" this Friday.</p>
                    </div>
                </div>
            </div>
        </div>
...
const Reports = () => (
    <Card>
        <h2 className="text-xl font-bold text-gray-800 mb-6">Reports & Certificates</h2>
            ...
            <div>
                ...
                <div className="space-y-3">
                    <button className="w-full text-left flex items-center space-x-3 p-3 bg-gray-50 hover:bg-gray-100 rounded-lg">
                        <FileText className="text-blue-600" /><span>Semester Marksheets</span>
                    </button>
                    <button className="w-full text-left flex items-center space-x-3 p-3 bg-gray-50 hover:bg-gray-100 rounded-lg">
                        <FileText className="text-green-600" /><span>Bonafide Certificate</span>
                    </button>
                    <button className="w-full text-left flex items-center space-x-3 p-3 bg-gray-50 hover:bg-gray-100 rounded-lg">
                        <FileText className="text-purple-600" /><span>Attendance Certificate</span>
                    </button>
                </div>
            </div>
            <div>
                ...
                <form className="space-y-4">
                    <div>
                        <label htmlFor="doc-type" className="text-sm text-gray-600">Document Type</label>
                    ...
                    <div>
                        <label htmlFor="reason" className="text-sm text-gray-600">Reason for Request</label>
                        <textarea id="reason" rows="3" className="w-full p-2 mt-1 border rounded-md"></textarea>
                    </div>
